...
from paper_experiments.vae_semg.supervised.model_diva_without_zd import DIVA_without_zd
    ...
    if work_method == 'train':
        for seed in range(10):
            ...
            for i in range(8):
                ...
                print(args.outpath)
                model_name = '../saved_model/diva_without_zd/' + 'test_domain_' + str(args.list_test_domain[0]) + '_diva_without_zd_seed_' + str(
                    args.seed)
                ...
                for epoch in range(1, args.epochs + 1):
                    ...
                    str_print += "     train accuracy y {}".format(train_accuracy_y)  # 训练集域分类精度
                    print(str_print)
                ...
                # this test accuracy is only for logging, this is not used
                # to make any decisions during training
                test_accuracy_y = get_accuracy(test_loader, model.classifier, args.batch_size)
    ...
    if work_method == 'test':
        ...
        for seed in range(10):
            ...
            for i in range(8):
                ...
                # to make any decisions during training
                test_accuracy_y = get_accuracy(test_loader, model.classifier, args.batch_size)
                print("Test_Subject", args.list_test_domain,test_accuracy_y)
                acc_in_repeat.append(test_accuracy_y*100)
            acc_all_repeat.append(acc_in_repeat)
        acc_all_repeat = np.array(acc_all_repeat)
        pd.DataFrame(acc_all_repeat).to_excel('1111.xlsx', sheet_name='0', index=False)

# test accuracy y 0.73677734375
# [0.6840122767857143, 0.8514716569767442, 0.7393032340116279, 0.90763671875, 0.90525390625, 0.7939245345744681, 0.73974609375, 0.73677734375]