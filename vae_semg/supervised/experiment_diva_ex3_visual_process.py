...
generate visualization data for
"""


def train(train_loader, model, optimizer, epoch, subject):
    model.train()
    train_loss = 0
    ...
    for batch_idx, (x, y, d) in enumerate(train_loader):
        ...
        # zd=zd_q.cpu().detach().numpy()
        zd, zy, zx, d, y = zd_q.cpu().detach().numpy(), \
                           zy_q.cpu().detach().numpy(), zx_q.cpu().detach().numpy(), \
                           d_target.cpu().detach().numpy(), y_target.cpu().detach().numpy()
        if batch_idx == 0:
            zd_list = zd
            zy_list = zy
            zx_list = zx
            ...
            zd_list = np.concatenate((zd_list, zd), axis=0)
            zy_list = np.concatenate((zy_list, zy), axis=0)
            zx_list = np.concatenate((zx_list, zx), axis=0)
            d_list = np.concatenate((d_list, d), axis=0)
            y_list = np.concatenate((y_list, y), axis=0)
    #
    # # 可视化
    # if epoch % 5 == 0:
    #     np.save("../../picture/ex3/train/zd_train.npy", [zd_list])
    #     np.save("../../picture/ex3/train/zy_train.npy", [zy_list])
    #     np.save("../../picture/ex3/train/zx_train.npy", [zx_list])
    #     np.save("../../picture/ex3/train/d_train.npy", [d_list])
    #     np.save("../../picture/ex3/train/y_train.npy", [y_list])
    # if epoch == 5:
    np.save("../../picture/addition/sub%d_zd_train_epoch%d.npy" % (subject, epoch), [zd_list])
    np.save("../../picture/addition/sub%d_zy_train_epoch%d.npy" % (subject, epoch), [zy_list])
    np.save("../../picture/addition/sub%d_zx_train_epoch%d.npy" % (subject, epoch), [zx_list])
    np.save("../../picture/addition/sub%d_d_train_epoch%d.npy" % (subject, epoch), [d_list])
    np.save("../../picture/addition/sub%d_y_train_epoch%d.npy" % (subject, epoch), [y_list])
...
def get_accuracy_for_tsne(data_loader, classifier_fn, batch_size):
    model.eval()
    """
    compute the accuracy over the supervised training set or the testing set
    """
    ...
    with torch.no_grad():
        # use the right data loader
        for batch_idx, (xs, ys, ds) in enumerate(data_loader):
            # To device
            xs, ys, ds = xs.to(device), ys.to(device), ds.to(device)

            # use classification function to compute all predictions for each batch
            pred_d, pred_y, zy = classifier_fn(xs)
            predictions_d.append(pred_d)
            actuals_d.append(ds)
            predictions_y.append(pred_y)
            actuals_y.append(ys)
            zy = zy.cpu().detach().numpy()
            ...
            if batch_idx == 0:
                ...
                zy_list = np.concatenate((zy_list, zy), axis=0)
                y_list = np.concatenate((y_list, y_label), axis=0)

        # zy_list=zy_list.tolist()
        # y_list=y_list.tolist()
        np.save("../../picture/ex3/test/zy_test.npy", [zy_list])
        np.save("../../picture/ex3/test/y_test.npy", [y_list])
    ...
    if work_method == 'train':
        for seed in range(1):
            ...
            for i in range(8):
                ...
                print(args.outpath)
                model_name = '../saved_model/diva_invarientd/' + 'test_domain_' + str(
                    args.list_test_domain[0]) + '_diva_10_seed_' + str(
                    args.seed)
                ...
                for epoch in range(1, args.epochs + 1):
                    ...
                    # train
                    avg_epoch_losses_sup, avg_epoch_class_y_loss = train(train_loader, model, optimizer, epoch, i)
    ...
    if work_method == 'test':
        ...
        for seed in range(1):
            ...
            for i in range(1):
                ...
                # Model name
                args.list_test_domain = [args.list_test_domain]
                model_name = '../saved_model/diva/' + 'test_domain_' + str(
                ...
                # this test accuracy is only for logging, this is not used
                # to make any decisions during training
                test_accuracy_d, test_accuracy_y = get_accuracy_for_tsne(test_loader, model.classifier_for_tsne,
                                                                         args.batch_size)
                print("test accuracy y {}".format(test_accuracy_y))
